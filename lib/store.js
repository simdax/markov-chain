    ...
    let setters = {
          ...
          for (var i = 0; i <= id; i++) {
          	state.edges.push({source:id,target:i,probability:Math.random().toFixed(precision)})
          };
          // populate old nodes edges
          for (var i = 0; i < id; i++) {
          	state.edges.push({source:i,target:id,probability:Math.random().toFixed(precision)})
          }
        },
